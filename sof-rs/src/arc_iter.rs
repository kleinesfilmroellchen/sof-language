//! Generic iterator over Arc<Vec<T>> that allows usage of IntoIter (with cloning of inner values) without having to re-allocate the reference-counted vector (which is what happens if you just call .into_iter on an Arc<Vec<T>>).
...
impl<'a, T> ArcVecIter<'a, T> {
    pub fn new(vec: Arc<Vec<T>>) -> Self {
        ...
        let end = if size_of::<T>() == 0 {
            begin.wrapping_byte_add(vec.len())
        } else {
            range.end
        };
        Self {
