...
class BuiltinFunctionsTest extends SofTestSuper {
	...
	void testRandomInt() {
		...
		for (int i = 0; i < 20000; ++i)
			assertDoesNotThrow(() -> randomInts.add(Builtins.random(IntPrimitive.createIntPrimitive(7l), IntPrimitive.createIntPrimitive(25l)).value()));

		assertTrue(randomInts.parallelStream().allMatch(i -> ((i >= 7l) && (i <= 25l))), "RNG generation limits");

		final double actualAverage = randomInts.parallelStream().collect(Collectors.averagingDouble(x -> x.doubleValue()));
		assertTrue(actualAverage >= avg - 1d && actualAverage <= avg + 1d, "Average value is approximately in the middle");

		assertTrue(randomInts.parallelStream().collect(Collectors.groupingByConcurrent(i -> i.longValue())).entrySet().parallelStream().map(entry -> entry.getValue().size())
				.allMatch(count -> count > avgCountPerNum * 0.8d && count <= avgCountPerNum * 1.2d), "All values are no more than 20% from the expected mean");
	}
	...
	void testInternalFmt() {
		...
		assertEquals("A is 3.558 and 0013 hehe le    ",
				Formatting.internalFormat("A is %.3f and %04d hehe %<6s", new Stackable[] { FloatPrimitive.createFloatPrimitive(3.55806d), IntPrimitive.createIntPrimitive(13l), StringPrimitive.createStringPrimitive("le") }),
